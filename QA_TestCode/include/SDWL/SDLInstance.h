...
public:
	...
	*/
	...
	virtual SDWLResult AddRPMDir(const std::wstring &filepath, uint32_t option = 0) = 0;
	...
	*	  option
	*			0		as normal
	*			1		set OVERWRITE_NORMAL flag on the new RPM folder. 
	*					When the RPM has this flag turn on, then when copying a NXL file to the new added RPM folder, driver will overwrite the normal file if exists.
	* @return
	*			SDWL_INVALID_DATA			invalid file path
	*			SDWL_CERT_NOT_READY         RPM driver/service is not ready
	*/

	virtual SDWLResult RemoveRPMDir(const std::wstring &filepath, bool bForce = false) = 0;
	...
	*/
	...
	virtual SDWLResult IsRPMFolder(const std::wstring &filepath, uint32_t* dirstatus) = 0;
	/// Check directory is RPM folder
	/**
	* @param
	*	  filepath		directory
	*     dirstatus     directory status, one or more RPM_SAFEDIRRELATION_xxx flags OR'ed together
	* @return
	*			SDWL_INVALID_DATA			invalid file path
	*			SDWL_CERT_NOT_READY         RPM driver/service is not ready
	*/
	...
	*/
	...
	virtual SDWLResult RPMGetFileRights(const std::wstring &filepath, std::vector<SDRmFileRight>& filerights, int option = 1) = 0;
	/**
	*  @param
	*	  filepath		file name
	*     rights        file rights
	...
	*  @param
	*	  filename		file name
	*     dirstatus     file status, one or more RPM_SAFEDIRRELATION_xxx flags OR'ed together
	...
	*/
	...
	virtual SDWLResult RPMResetRegistryOfClick2Run() = 0;
	/// Ask to reset the registries of Office Click2Run
	/**
	* @return
	*           SDWL_INTERNAL_ERROR         Error like User not login
	*/
};
