...
public:
    ...
    public:
        ...
        std::set<std::wstring> get(const std::wstring& key) const;
        void remove(const std::wstring& key);
        void remove(const std::wstring& key, const std::wstring& value);

        std::wstring serialize() const;
        static Tags deserialize(const std::wstring& s);

    private:
        std::unordered_map<std::wstring, std::set<std::wstring>> data;
    };
...
protected:
    ...
    FileError readSection(const std::string& name, std::vector<uint8_t>& data);
