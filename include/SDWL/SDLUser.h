...
public:
	...
	*/
	...
	virtual SDWLResult GetLocalFileManager(ISDRFiles** pFile) = 0;
	...
	*
	*/
	virtual SDWLResult ShareFile(const std::wstring &filepath, std::wstring& newcreatedfilePath,
		const std::vector<SDRmFileRight> &rights, const std::vector<std::string> &recipientsemail, const std::wstring & comments, const SDR_WATERMARK_INFO &watermarkinfo, const SDR_Expiration &expire, const std::string& tags = "", const std::string& memberid = "" ) = 0;
	/// Share a local file
	/**
	* @param
	*    filepath			full path to original file
	...
	*/
	...
	virtual SDWLResult UpdateRecipients(ISDRmNXLFile * nxlfile, const std::vector<std::string> &addrecipientsemail, const std::vector<std::string> &removerecipientsemail) = 0;
	...
	*/
	...
	virtual SDWLResult UploadFile(ISDRmNXLFile* file) = 0;
	/// upload  local file 
	/**
	* @param
	...
	*/
	...
	virtual SDWLResult UploadActivityLogs() = 0;
	...
	*/
	...
	virtual SDWLResult GetNXLFileActivitylog(ISDRmNXLFile * file, uint64_t startPos, uint64_t count, uint8_t searchField, const std::string &searchText, uint8_t orderByField, bool orderByReverse) = 0;
	...
	*/
	...
	virtual SDWLResult DeleteSavedTokens() = 0;
	/// Delete saved tokens
	/**
	* @return
	*            SDWL_INVALID_DATA       Invalid data
	...
	*/
	...
	virtual SDWLResult IsFileRevoked(const std::wstring &nxlfilepath, bool& revoke) = 0;
	/// Get file path from file name
	/**
	* @param
	*    nxlfilepath  file name
	*    revoke    if file is revoked, return true
	* @return
	*            SDWL_INVALID_DATA       Invalid data
	*            SDWL_RMS_ERRORCODE_BASE+	RMS server error
	*
	...
	*/
	...
	virtual SDWLResult PDDecryptPartial(unsigned char* in, long in_len, long offset, unsigned char* out, long* out_len, unsigned char* header, long header_len) = 0;
	...
	*/
	...
	virtual int64_t  PDGetContentLength() = 0;
	...
	*/
	...
	virtual SDWLResult PDSetupHeader(unsigned char* header, long header_len, int64_t* contentLength, unsigned int* contentOffset) = 0;
	/// Decrypt partial file data
	/**
	* @param
	*    header  nxl file header
